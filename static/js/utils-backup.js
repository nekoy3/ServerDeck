    ...
    modalManager: {
        ...
        cleanupModal: function(modalElement) {
            ...
            try {
                ...
                console.warn(`⚠️  [MODAL] Error resetting DOM state for modal ${modalId}:`, error);
            }
        ...
        cleanupAllModals: function() {
            ...
            document.querySelectorAll('.modal').forEach(modalElement => {
                if (!modalElement) return; // 安全性チェック
                ...
                try {
                    ...
                    console.warn(`⚠️  [MODAL] Error resetting DOM state for ${modalElement.id}:`, e.message);
                }
            });
            ...
            try {
                ...
                console.warn('⚠️  [MODAL] Error removing backdrop:', e.message);
            }
            ...
            try {
                ...
                console.warn('⚠️  [MODAL] Error resetting body state:', e.message);
            }
        ...
        setTimeout(() => {
            ...
            fetch('/config')
                ...
                .then(html => {
                    ...
                    // さらに少し待ってからモーダルを開く（DOMコンテンツが完全に準備されるまで）
                    setTimeout(() => {
                        try {
                            // モーダルを開く（エラーを安全に処理）
                            ...
                            if (modalInstance) {
                                ...
                                configModalElement.addEventListener('shown.bs.modal', () => {
                                    ...
                                    if (typeof window.loadServersForConfigModal === 'function') {
                                        console.log('📡 [CONFIG] Loading servers for config modal');
                                        window.loadServersForConfigModal();
                                    } else {
                                        console.warn('⚠️ [CONFIG] window.loadServersForConfigModal function not found');
                                    }
                        
                        // バックアップ管理を初期化（バックアップタブの内容を確実に表示）
                        if (typeof window.BackupManagement !== 'undefined') {
                            console.log('💾 [CONFIG] Initializing backup management');
                            try {
                                window.BackupManagement.initialize();
                                if (typeof window.BackupManagement.loadBackupFileList === 'function') {
                        ...
                        if (typeof window.SshKeyManagement !== 'undefined') {
                            ...
                            try {
                                ...
                                // グローバル関数を使用してSSH鍵リストを読み込み
                                    ...
                                    if (typeof window.SshKeyManagement !== 'undefined') {
                                        ...
                                        try {
                                            window.SshKeyManagement.initialize();
                                            // グローバル関数を使用してSSH鍵リストを読み込み
                                            if (typeof window.loadSshKeysForManagementModal === 'function') {
                                                console.log('🔑 [CONFIG] Loading SSH keys for management modal (modal shown)');
                                                window.loadSshKeysForManagementModal();
                                            } else {
                                                console.warn('⚠️ [CONFIG] window.loadSshKeysForManagementModal function not found');
                                            }
                                        } catch (error) {
                                ...
                                console.warn('⚠️ [CONFIG] Modal instance not created successfully');
                            }
                        } catch (modalError) {
                            console.error('❌ [CONFIG] Error creating modal instance:', modalError);
                        }
                    }, 50); // DOM準備完了まで少し待つ
                })
                .catch(error => {
    ...
    initializeConfigModalScripts: function() {
        ...
        if(serverTab) {
            serverTab.addEventListener('show.bs.tab', () => {
                if (typeof window.loadServersForConfigModal === 'function') {
                    console.log('📡 [CONFIG] Loading servers for config modal (tab switch)');
                    window.loadServersForConfigModal();
                } else {
                    console.warn('⚠️ [CONFIG] window.loadServersForConfigModal function not found');
                }
        ...
        if (sshKeysTab) {
            sshKeysTab.addEventListener('shown.bs.tab', () => {
                if (window.SshKeyManagement && typeof window.SshKeyManagement.initialize === 'function') {
                    console.log('🔑 [CONFIG] Initializing SSH key management (tab switch)');
                    window.SshKeyManagement.initialize();
                    // SSH鍵リストも読み込み（グローバル関数使用）
                    if (typeof window.loadSshKeysForManagementModal === 'function') {
                        console.log('🔑 [CONFIG] Loading SSH keys list');
                        window.loadSshKeysForManagementModal();
                    } else {
                        console.warn('⚠️ [CONFIG] window.loadSshKeysForManagementModal function not found');
                    }
                } else {
                    console.warn('⚠️ [CONFIG] window.SshKeyManagement not found or missing initialize method');
                }
            }, { once: true });
        }
        ...
        if (extraImportTab) {
            ...
            extraImportTab.addEventListener('shown.bs.tab', () => {
                if (window.ExtraImport && typeof window.ExtraImport.initialize === 'function') {
                    console.log('📥 [CONFIG] Initializing extra import (tab switch)');
                    window.ExtraImport.initialize();
                } else {
                    console.warn('⚠️ [CONFIG] window.ExtraImport not found or missing initialize method');
                }
            }, { once: true });
        ...
        if (backupTab) {
            backupTab.addEventListener('shown.bs.tab', () => {
                if (window.BackupManagement && typeof window.BackupManagement.initialize === 'function') {
                    console.log('🔧 [CONFIG] Initializing backup management (tab switch)');
                    window.BackupManagement.initialize();
                    // バックアップリストを確実に読み込み
                    if (typeof window.BackupManagement.loadBackupFileList === 'function') {
                        window.BackupManagement.loadBackupFileList();
                    ...
                    console.warn('⚠️ [CONFIG] window.BackupManagement not found or missing methods');
                }
            }, { once: true });
        }
