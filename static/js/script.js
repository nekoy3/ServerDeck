document.addEventListener('DOMContentLoaded', function() {
    ...
    if (configLink) {
        configLink.addEventListener('click', function(e) {
            e.preventDefault();
            // 新しいモジュール構成を使用
            if (window.ServerDeckUtils && window.ServerDeckUtils.openConfigModal) {
                window.ServerDeckUtils.openConfigModal();
            } else {
                console.error('ServerDeckUtils not available for config modal');
            }
        });
    }
    ...
    window.updateMainPageServerCards = function updateMainPageServerCards() {
        fetch('/api/servers')
            .then(response => response.json())
            .then(servers => {
    ...
    function handleEditServerClick() {
        ...
        if (configModal) {
            configModal.hide();
            // モーダルが完全に閉じられてから編集モーダルを開く（設定モーダルから開いたことを示すフラグ付き）
            setTimeout(() => {
                openEditModalForServer(serverId, true); // fromConfigModal = true
    ...
    // サーバー編集モーダルを開く共通関数
    function openEditModalForServer(serverId, fromConfigModal = false) {
        // ServerManagement.openEditModal を使用してfromConfigModalパラメータを正しく渡す
        if (window.ServerManagement && window.ServerManagement.openEditModal) {
            window.ServerManagement.openEditModal(serverId, fromConfigModal);
            return;
        }
        ...
        fetch(`/api/servers/${serverId}`)
            ...
            .then(server => {
                ...
                // 親ホスト選択フィールドのセットアップ
                if (window.ServerManagement && typeof window.ServerManagement.setupParentHostFieldForConfig === 'function') {
                    window.ServerManagement.setupParentHostFieldForConfig(server);
                }

                // ServerManagement.openEditModalを使用してモーダルを開く
                if (window.ServerManagement && typeof window.ServerManagement.openEditModal === 'function') {
                    // 既存のモーダルを閉じる
                    editModal.hide();
                    // ServerManagement.openEditModalを呼び出す
                    window.ServerManagement.openEditModal(serverId, fromConfigModal);
                } else {
                    // フォールバック: 直接モーダルを表示
                    editModal.show();
    ...
    function initializeConfigModalScripts() {
        ...
        // 新規サーバー追加時に親ホスト選択フィールドを初期化
        if (window.ServerManagement && typeof window.ServerManagement.setupParentHostFieldForConfig === 'function') {
            window.ServerManagement.setupParentHostFieldForConfig({ parent_id: null });
        }
    ...
    window.loadServersForConfigModal = function loadServersForConfigModal() {
        fetch('/api/servers')
            .then(response => response.json())
            .then(servers => {
                ...
                fetch(`/api/servers/${serverId}`)
                    ...
                    .then(server => {
                        ...
                        editServerForm.onsubmit = function(e) {
                            ...
                            // 親ホスト（parent_id）の値を追加
                            const parentHostSelect = document.getElementById('parentHost');
                            if (parentHostSelect) {
                                payload.parent_id = parentHostSelect.value || null;
                            }
